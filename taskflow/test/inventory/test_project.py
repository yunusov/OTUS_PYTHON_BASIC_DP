...
def test_get_project(projects_api_url, client, project, token):
...
def test_update_project(projects_api_url, client, project, token):
    project_id = project["id"]
    update_json = {
...
def test_delete_project(projects_api_url, client, project, token):
    project_id = project["id"]
    logger.info(projects_api_url % project_id)
    resp = client.delete(
        projects_api_url % project_id,
        headers={"Authorization": f"Bearer {token}"},
    )
...
def test_add_members(
    ...
    add_members_json = {
        "user_ids": [user["id"] for user in project_users],
    }
